...
@if($withGallery != 3)
  ...
  #{{$id}} .custom-item-gallery .gallery .owl-nav {
    & i {
      ...
      &:hover { color: {{$galleryNavColorHover}}; }
    }
    & [class*=owl-]:hover { background: transparent; }
    @if($galleryLayout == "gallery-layout-1")
      margin-top: 0;
      @if($galleryNavPosition == 2) text-align: right; @endif
      @if($galleryNavPosition == 3) text-align: left; @endif
...
#{{$id}} .custom-item-share .share {
  @if(!empty($shareStyle))
   {!! $shareStyle !!}
  @endif
  @if(!empty($shareFontClass))
  & .share-text { font-size: {{$shareFontClass}}px; }
  @endif
}
@endif
...
@if( in_array($withBodyExtra, [1, 2]))
#{{$id}} .custom-item-body-extra .body-extra {
 @if(!empty($bodyExtraFontSize)) font-size: {{$bodyExtraFontSize}}px; @endif
 @if($bodyExtraColorByClass=="text-custom")  color: {{$bodyExtraColor}}; @endif
 @if(!empty($bodyExtraStyle))
 {!! $bodyExtraStyle !!}
 @endif
}
@endif

@if(in_array($withVideoExternal, [1, 2]) && !empty($videoExternalStyle))
