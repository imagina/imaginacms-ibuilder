...
namespace Modules\Ibuilder\Http\Livewire;

use Livewire\Component;

class LWContentCustom extends Component
{
  public $customId;
  public $readyToLoad;
  public $item;
  ...
  public $itemListTitleClass;
  public $itemListTitleH;
  public $itemListTitlePage;
  public $itemListTake;
  public $itemListCol;
  ...
  public $itemListPagOrderBy;
  public $itemListPagOrderByOn;
  public $view;
  ...
  public function mount($customId = null,
                        $item = null,
                        $viewParams = [],
                        $row = "row",
                        $withTitle = 1,
  ...
  )
  {
    ...
    $this->customId = $customId ?? uniqid('customItemL');
    $this->row = $row;
    ...
    $this->itemListTitleClass = $itemListTitleClass;
    $this->itemListTitleH = $itemListTitleH;
    $this->itemListTitlePage = $itemListTitlePage;
    ...
    $this->itemListPagOrderBy = !empty($itemListPagOrderBy) ? $itemListPagOrderBy : [
      'default' => 'recently',
      'options' => [
        'nameaz' => [
    ...
    $this->itemListPagOrderByOn = $itemListPagOrderByOn;
    ...
    $this->imageZoneName = $imageZone;
    $this->galleryZoneName = $galleryZone;
    $this->imageZone = $this->getZone($imageZone, $this->item, $this->typeContent, 'private');
    $this->galleryZone = $this->getZone($galleryZone, $this->item, $this->typeContent, 'premium');
  ...
  public function refreshZoneData()
  {
    $this->imageZone = $this->getZone($this->imageZoneName, $this->item, $this->typeContent, 'private');
    $this->galleryZone = $this->getZone($this->galleryZoneName, $this->item, $this->typeContent, 'premium');
   ...
   * @return imgZone
   */
  public function getZone($zone, $item, $typeContent, $text)
  {
    $imgZone = str_replace($text, "", $zone);
    if ($typeContent == 'ad') {
      $allowPrivateMedia = method_exists($item, 'allowPrivateMedia') ? $item->allowPrivateMedia() : false;
      if ($allowPrivateMedia) {
        $mediaFiles = $item->mediaFiles();
        $privateZone = $imgZone . $text;
        if (isset($mediaFiles->$privateZone)) $imgZone = $privateZone;
      }
    }
    return $imgZone;
  }
  ...
  public function getItem($item, $params, $withFilterCategory, $withCarousel, $withListLateral)
  {
    ...
    switch ($this->item->entity) {
      case 'Modules\Page\Entities\Page':
        ...
        $this->view = "ibuilder::frontend.livewire.lw-content-custom.page.index";
      ...
      case 'Modules\Iblog\Entities\Post':
        ...
        $this->view = "ibuilder::frontend.livewire.lw-content-custom.blog.index";
      ...
      case 'Modules\Iad\Entities\Ad':
        $this->typeContent = 'ad';
        $this->view = "ibuilder::frontend.livewire.lw-content-custom.ad.index";
        $this->withFilterCategory = $withFilterCategory ?? true;
        $this->withCarousel = $withCarousel ?? true;
        $this->withListLateral = $withListLateral ?? true;
  ...
  public function getFilters($filter, $title)
  {
    ...
    if (is_null($filter)) {
      $filter = ['title' => $title,
        ...
        'isExpanded' => true,
        'type' => 'tree',
        'repository' => $repository ?? 'Modules\Iblog\Repositories\CategoryRepository',
        'entityClass' => $entityClass ?? 'Modules\Iblog\Entities\Category',
